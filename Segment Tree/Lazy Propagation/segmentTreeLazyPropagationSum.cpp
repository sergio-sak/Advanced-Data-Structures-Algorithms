...
int f(int a, int b) {
    return (a + b); // Any other operation which segtree supports at that    
}
...
void Propagate(int ind, int l, int r) {
    ...
    seg[2*ind] += lazy[ind] * (mid-l+1);
    seg[2*ind+1] += lazy[ind] * (r-mid);
...
void Update(int from, int to, int val, int ind = 1, int l = 0, int r = N-1) { //Range Updates
    ...
    Propagate(ind, l, r); //Propagating is not needed for sum queries but the code should be modified a bit. For that reason I prefer propagating
    int mid = l + (r-l)/2;
